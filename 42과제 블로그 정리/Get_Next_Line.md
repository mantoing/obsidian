...
거의 c언어를 처음 시작할때 시작했던 과제였는데 힘겹게 겨우겨우 공부하면서 구현했던 기억이 있다. linked-list로 구현을 하고싶어 처음에 구현을 시작했지만 리스트에 대한 개념과 공부가 너무 부족해서 건들지 못할거 같아서 open_max로 구현을 하였다. write 함수에서 첫번째 인자가 무슨 의미인지 모르고 생각없이 사용을 했었는데 파일 디스크립터에 대해서 공부를 하니 이해하지 못했던 코드들과 내가 구현했던 코드의 오류도 찾아낼수 있어서 재밌는 시간이었다.
